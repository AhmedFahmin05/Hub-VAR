...
dimacs_graph load_dimacs_graph(
        const std::string&dimacs,
        const std::function<void(const std::string&)>&log_message
){
    dimacs_graph dimacsGraph;
    dimacsGraph.first_out = load_vector<unsigned>(dimacs+".first");
    dimacsGraph.head = load_vector<unsigned>(dimacs+".head");
    dimacsGraph.weight = load_vector<unsigned>(dimacs+".weight");
    dimacsGraph.latitude = load_vector<float>(dimacs+".latitude");
    dimacsGraph.longitude = load_vector<float>(dimacs+".longitude");
    return dimacsGraph;
}
} // RoutingKit
